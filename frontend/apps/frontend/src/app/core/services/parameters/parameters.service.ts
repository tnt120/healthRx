...
export class ParametersService {
  ...
  getUserParameters(): Observable<UserParameterResponse[]> {
    ...
    return this.http
      .get<UserParameterResponse[]>(`${this.apiUrl}/users`)
      .pipe(finalize(() => this.setLoadingState(false)));
  }

  editUserParameters(
    parameters: Parameter[]
  ): Observable<UserParameterResponse[]> {
    return this.http
      .patch<UserParameterResponse[]>(`${this.apiUrl}/users`, parameters)
      .pipe(
        tap(() => {
          this.customSnackbarService.openCustomSnackbar({
            title: 'Sukces',
            message: 'Pomyślnie zaktualizowano parametry',
            type: 'success',
            duration: 3000,
          });
        }),
        catchError((err) => {
          this.customSnackbarService.openCustomSnackbar({
            title: 'Błąd',
            message: 'Nie udało się zaktualizować parametrów.',
            type: 'error',
            duration: 3000,
          });
  ...
  setUserParametersMonitor(
    request: UserParameterRequest[]
  ): Observable<UserParameterResponse[]> {
    return this.http.post<UserParameterResponse[]>(
      `${this.apiUrl}/monitor`,
      request
    );
  }

  editUserParametersMonitor(
    request: UserParameterRequest
  ): Observable<UserParameterResponse> {
    return this.http.patch<UserParameterResponse>(
      `${this.apiUrl}/monitor`,
      request
    );
  }
