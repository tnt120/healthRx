...
export class ActivityService {
  ...
  private loadingActivitiesSubject = new BehaviorSubject<
    Map<'today' | 'all', boolean>
  >(
    new Map([
      ['today', false],
      ['all', false],
  ...
  getLoadingActivityState(type: 'today' | 'all'): Observable<boolean> {
    return this.loadingActivitiesSubject
      .asObservable()
  ...
  getActivities(
    ...
    size: number,
    sort: SortOption,
    search?: AcitivtySearchParams
  ): Observable<PageResponse<UserActivityResponse>> {
    ...
    if (search?.activityId)
      params = params.set('activityId', search.activityId);
    ...
    const type: 'today' | 'all' =
      search?.startDate?.substring(0, 10) === search?.endDate?.substring(0, 10)
        ? 'today'
        : 'all';

    this.setLoadingActivityState(type, true);
    return this.http
      .get<PageResponse<UserActivityResponse>>(`${this.apiUrl}/user`, {
        params,
      })
      .pipe(
        map((res) => ({
  ...
  addUserActivity(req: UserActivityRequest): Observable<UserActivityResponse> {
    return this.http
      .post<UserActivityResponse>(`${this.apiUrl}/user`, req)
      .pipe(
        map((activity) => ({
          ...activity,
          activityTime: new Date(activity.activityTime),
        ...
        catchError((err) => {
          this.customSnackbarService.openCustomSnackbar({
            title: 'Błąd',
            message: 'Wystawił błąd podczas dodwania aktywności.',
            type: 'error',
            duration: 2500,
          });
  ...
  editUserActivity(
    ...
    return this.http
      .put<UserActivityResponse>(`${this.apiUrl}/user/${id}`, req)
      .pipe(
        map((activity) => ({
          ...activity,
          activityTime: new Date(activity.activityTime),
        ...
        catchError((err) => {
          this.customSnackbarService.openCustomSnackbar({
            title: 'Błąd',
            message: 'Wystawił błąd podczas edytowania aktywności.',
            type: 'error',
            duration: 2500,
          });
          return throwError(() => err);
  ...
  deleteUserActivity(id: string): Observable<void> {
    return this.http.delete<void>(`${this.apiUrl}/user/${id}`).pipe(
      ...
      catchError((err) => {
        this.customSnackbarService.openCustomSnackbar({
          title: 'Błąd',
          message: 'Wystawił błąd podczas usuwania aktywności.',
          type: 'error',
          duration: 2500,
        });
        return throwError(() => err);
