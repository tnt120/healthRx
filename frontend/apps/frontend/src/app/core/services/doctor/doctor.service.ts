...
export class DoctorService {
  ...
  private doctorBehaviorSubject = new BehaviorSubject<DoctorResponse[] | null>(
    null
  );
  ...
  getDoctors(
    ...
    if (doctorSearch.firstName)
      params = params.set('firstName', doctorSearch.firstName);
    if (doctorSearch.lastName)
      params = params.set('lastName', doctorSearch.lastName);
    if (doctorSearch.specialization)
      params = params.set('specialization', doctorSearch.specialization);
    if (doctorSearch.city) params = params.set('city', doctorSearch.city);

    this.setLoadingDoctorsState(true);
    return this.http
      .get<PageResponse<DoctorResponse>>(this.apiUrl, { params })
      .pipe(
        tap((res) => {
