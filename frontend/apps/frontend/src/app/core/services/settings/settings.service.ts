...
export class SettingsService {
  ...
  passwordChange(request: PasswordChangeRequest): Observable<void> {
    return this.http.post<void>(`${this.apiUrl}/passwordChange`, request).pipe(
      tap(() => {
        this.customSnackbarService.openCustomSnackbar({
          title: 'Sukces',
          message: 'Pomyślnie zaktualizowano hasło',
          type: 'success',
          duration: 3000,
        });
      }),
  ...
  notificationsChange(
    ...
    return this.http
      .put<NotificationsData>(`${this.apiUrl}/notificationsChange`, request)
      .pipe(
        tap(() => {
          this.customSnackbarService.openCustomSnackbar({
            title: 'Sukces',
            message: 'Pomyślnie zaktualizowano ustawienia powiedomień',
            type: 'success',
            duration: 3000,
          });
        }),
        catchError((err) => {
          this.customSnackbarService.openCustomSnackbar({
            title: 'Błąd',
            message: 'Nie udało się ustawień powiadomień.',
            type: 'error',
            duration: 3000,
          });
          return throwError(() => err);
