...
export class StatisticsServiceService {
  ...
  private loadingStatsSubject = new BehaviorSubject<
    Map<StatisticsType, boolean>
  >(Statistics_Type_Init);

  private loadingChartSubject = new BehaviorSubject<
    Map<StatisticsType, boolean>
  >(Statistics_Type_Init);

  getLoadingStatsState(type: StatisticsType): Observable<boolean> {
    return this.loadingStatsSubject
      .asObservable()
      .pipe(map((stateMap) => stateMap.get(type) || false));
  }
  ...
  getLoadingChartState(type: StatisticsType): Observable<boolean> {
    return this.loadingChartSubject
      .asObservable()
      .pipe(map((stateMap) => stateMap.get(type) || false));
  }
  ...
  getChartData(req: ChartRequest): Observable<ChartResponse> {
    const type: StatisticsType =
      StatisticsType[req.type as keyof typeof StatisticsType];

    this.setLoadingChartState(type, true);

    return this.http
      .post<ChartResponse>(`${this.apiUrl}/chart`, req)
      .pipe(finalize(() => this.setLoadingChartState(type, false)));
  }

  getParameterStatistics(
    req: StatisticsRequest
  ): Observable<ParameterStatisticsResponse[]> {
    this.setLoadingStatsState(StatisticsType.PARAMETER, true);
    return this.http
      .post<ParameterStatisticsResponse[]>(`${this.apiUrl}/parameters`, req)
      .pipe(
        finalize(() =>
          this.setLoadingStatsState(StatisticsType.PARAMETER, false)
        )
      );
  }
  ...
  getDrugsStatistics(
    req: StatisticsRequest
  ): Observable<DrugStatisticsResponse[]> {
    this.setLoadingStatsState(StatisticsType.DRUG, true);
    return this.http
      .post<DrugStatisticsResponse[]>(`${this.apiUrl}/drugs`, req)
      .pipe(
        finalize(() => this.setLoadingStatsState(StatisticsType.DRUG, false))
      );
  }

  getAcitvityStatistics(
    req: StatisticsRequest
  ): Observable<ActivityStatisticsResponse[]> {
    this.setLoadingStatsState(StatisticsType.ACTIVITY, true);
    return this.http
      .post<ActivityStatisticsResponse[]>(`${this.apiUrl}/activities`, req)
      .pipe(
        finalize(() =>
          this.setLoadingStatsState(StatisticsType.ACTIVITY, false)
        )
      );
  }
  ...
  generateReport(req: GenerateReportRequest): Observable<Blob> {
    ...
    return this.http
      .post<Blob>(`${this.apiUrl}/generate-report`, req, {
        headers: headers,
      ...
      .pipe(
        catchError((err) => {
          ...
          this.customSnackbarService.openCustomSnackbar({
            title: 'Błąd',
            message:
              'Wystawił błąd podczas generowania raportu. Spróbuj ponownie za chwilę.',
            type: 'error',
            duration: 2500,
          });
