...
import {
  ...
  FriendshipResponse,
} from '../../models/friendship-response.model';
...
export class FriendshipService {
  ...
  private loadingFriendshipsPendingSubject = new BehaviorSubject<boolean>(
    false
  );
  private loadingFriendshipsRejectedSubject = new BehaviorSubject<boolean>(
    false
  );

  loadingFriendships$ = this.loadingFriendshipsSubject.asObservable();
  loadingFriendshipsPending$ =
    this.loadingFriendshipsPendingSubject.asObservable();
  loadingFriendshipsRejected$ =
    this.loadingFriendshipsRejectedSubject.asObservable();
  ...
  getFriendships(
    ...
    sort: SortOption,
    size: number,
    friendshipSearch: FriendshipSearch
  ): Observable<PageResponse<FriendshipResponse>> {
    ...
    if (friendshipSearch.firstName)
      params = params.set('firstName', friendshipSearch.firstName);
    if (friendshipSearch.lastName)
      params = params.set('lastName', friendshipSearch.lastName);

    this.setLoadingFriendshipsState(true);
    return this.http
      .get<PageResponse<FriendshipResponse>>(`${this.apiUrl}/accepted`, {
        params,
      })
      .pipe(
        tap((res) => {
  ...
  ): Observable<FriendshipPermissions> {
    return this.http
      .post<FriendshipPermissions>(
        `${this.apiUrl}/permissions/${friendshipId}`,
        permissions
      )
      .pipe(
        tap(() => {
          this.customSnackbarService.openCustomSnackbar({
            title: 'Sukces',
            message: 'Pomyślnie edytowano upraweniania.',
            type: 'success',
            duration: 3000,
          });
        ...
        catchError((err) => {
          this.customSnackbarService.openCustomSnackbar({
            title: 'Błąd',
            message: 'Wystawił błąd podczas edytowania uprawnień.',
            type: 'error',
            duration: 3000,
          });
  ...
  updateFriendships(
    friendshipId: string,
    action: 'delete' | 'permissionUpdate',
    newPermissions: FriendshipPermissions | null = null
  ) {
    const currentFrienships = this.friendshipsSubject.getValue();

    switch (action) {
      case 'delete': {
        const updatedFriendships = currentFrienships.filter(
  ...
  getFriendshipsPending(): Observable<FriendshipResponse[]> {
    ...
    return this.http
      .get<FriendshipResponse[]>(`${this.apiUrl}/pending`)
      .pipe(finalize(() => this.setLoadingFriendshipsPendingState(false)));
  }

  getFriendshipsRejected(): Observable<FriendshipResponse[]> {
    this.setLoadingFriendshipsRejectedState(true);
    return this.http
      .get<FriendshipResponse[]>(`${this.apiUrl}/rejected`)
      .pipe(finalize(() => this.setLoadingFriendshipsRejectedState(false)));
  }

  sendInvitation(request: InvitationRequest): Observable<InvitationResponse> {
    return this.http
      .post<InvitationResponse>(`${this.apiUrl}/invite`, request)
      .pipe(
        tap(() => {
          this.customSnackbarService.openCustomSnackbar({
            title: 'Sukces',
        ...
        catchError((err) => {
          this.customSnackbarService.openCustomSnackbar({
            title: 'Błąd',
            message: 'Wystawił błąd podczas wysyłania zaproszenia.',
            type: 'error',
            duration: 3000,
          });
          return throwError(() => err);
  ...
  acceptInvitation(request: InvitationRequest): Observable<InvitationResponse> {
    return this.http
      .post<InvitationResponse>(`${this.apiUrl}/accept`, request)
      .pipe(
        tap(() => {
          this.customSnackbarService.openCustomSnackbar({
            title: 'Sukces',
        ...
        catchError((err) => {
          this.customSnackbarService.openCustomSnackbar({
            title: 'Błąd',
            message: 'Błąd podczas akcpetacji zaproszenia.',
            type: 'error',
            duration: 3000,
          });
          return throwError(() => err);
  ...
  rejectInvitation(request: InvitationRequest): Observable<InvitationResponse> {
    return this.http
      .post<InvitationResponse>(`${this.apiUrl}/reject`, request)
      .pipe(
        tap(() => {
          this.customSnackbarService.openCustomSnackbar({
            title: 'Sukces',
        ...
        catchError((err) => {
          this.customSnackbarService.openCustomSnackbar({
            title: 'Błąd',
            message: 'Błąd podczas odrzucenia zaproszenia.',
            type: 'error',
            duration: 3000,
          });
          return throwError(() => err);
  ...
  resendInvitation(request: InvitationRequest): Observable<InvitationResponse> {
    return this.http
      .post<InvitationResponse>(`${this.apiUrl}/resend`, request)
      .pipe(
        tap(() => {
          this.customSnackbarService.openCustomSnackbar({
            title: 'Sukces',
            message: 'Ponowne zaprosznie zostało pomyślnie wysłane.',
            type: 'success',
            duration: 3000,
          });
        }),
  ...
  cancelInvitation(
    ...
    return this.http
      .delete<InvitationResponse>(`${this.apiUrl}/remove/${id}`)
      .pipe(
        tap(() => {
          const successMessage = isAccepted
            ? 'Usunięto użytkownika ze znajomych.'
        ...
        catchError((err) => {
          this.customSnackbarService.openCustomSnackbar({
            title: 'Błąd',
            message: 'Błąd podczas usuwania ze znajomych.',
            type: 'error',
            duration: 3000,
          });
          return throwError(() => err);
