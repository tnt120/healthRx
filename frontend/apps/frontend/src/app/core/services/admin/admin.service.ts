...
export class AdminService {
  ...
  getDashboardData(): Observable<DashboardDataResponse> {
    ...
    return this.http
      .get<DashboardDataResponse>(`${this.apiUrl}/dashboard`)
  ...
  getApprovals(
    ...
    this.loadingSubject.next(true);
    return this.http
      .get<PageResponse<DoctorResponse>>(`${this.apiUrl}/approvals`, { params })
      .pipe(
    ...
    return this.http.patch<void>(`${this.apiUrl}/verifyDoctor`, req).pipe(
      tap(() => {
        this.customSnackbarService.openCustomSnackbar({
          title: 'Sukces',
          message: 'Pomyślnie zaweryfikowano lekarza.',
          type: 'success',
          duration: 3000,
        });
        const currApprovals: DoctorResponse[] =
          this.approvalsSubject.getValue();

        this.approvalsSubject.next(
          currApprovals.filter((doc) => doc.id !== req.doctorId)
        );
      }),
      catchError((err) => {
  ...
  getParameters(): Observable<AdminParameterResponse> {
    this.loadingSubject.next(true);
    return this.http
      .get<AdminParameterResponse>(`${this.apiUrl}/parameters`)
      .pipe(finalize(() => this.loadingSubject.next(false)));
  }
  ...
  addParameter(req: ParameterRequest): Observable<Parameter> {
    return this.http.post<Parameter>(`${this.apiUrl}/parameters`, req).pipe(
      tap(() => {
        this.customSnackbarService.openCustomSnackbar({
          title: 'Sukces',
          message: 'Pomyślnie dodano parametr.',
          type: 'success',
          duration: 3000,
        });
      }),
  ...
  editParameter(
    req: Partial<ParameterRequest>,
    id: string
  ): Observable<Parameter> {
    return this.http
      .patch<Parameter>(`${this.apiUrl}/parameters/${id}`, req)
      .pipe(
        tap(() => {
          this.customSnackbarService.openCustomSnackbar({
            title: 'Sukces',
            message: 'Pomyślnie edytowano parametr.',
            type: 'success',
            duration: 3000,
          });
        }),
  ...
  deleteParameter(id: string): Observable<void> {
    return this.http.delete<void>(`${this.apiUrl}/parameters/${id}`).pipe(
      tap(() => {
        this.customSnackbarService.openCustomSnackbar({
          title: 'Sukces',
          message: 'Pomyślnie usunięto parametr.',
          type: 'success',
          duration: 3000,
        });
      }),
  ...
  editActivity(
    req: Partial<ActivityRequest>,
    id: string
  ): Observable<Activity> {
    return this.http
      .patch<Activity>(`${this.apiUrl}/activities/${id}`, req)
      .pipe(
        tap(() => {
          this.customSnackbarService.openCustomSnackbar({
            title: 'Sukces',
            message: 'Pomyślnie edytowano aktywność.',
            type: 'success',
            duration: 3000,
          });
        }),
  ...
  deleteActivity(id: string): Observable<void> {
    return this.http.delete<void>(`${this.apiUrl}/activities/${id}`).pipe(
      tap(() => {
        this.customSnackbarService.openCustomSnackbar({
          title: 'Sukces',
          message: 'Pomyślnie usunięto aktywność.',
          type: 'success',
          duration: 3000,
        });
      }),
  ...
  getUsers(
    ...
    if (userSearchFilters.role)
      params = params.set('role', userSearchFilters.role);

    if (userSearchFilters.firstName)
      params = params.set('firstName', userSearchFilters.firstName);

    if (userSearchFilters.lastName)
      params = params.set('lastName', userSearchFilters.lastName);

    this.loadingSubject.next(true);
    return this.http
      .get<PageResponse<UserResponse>>(`${this.apiUrl}/users`, { params })
      .pipe(
  ...
  changeUserRole(
    req: ChangeRoleReqRes,
    id: string
  ): Observable<ChangeRoleReqRes> {
    return this.http
      .patch<ChangeRoleReqRes>(`${this.apiUrl}/users/${id}`, req)
      .pipe(
        tap(() => {
  ...
  deleteUser(req: DeleteUserRequest, id: string): Observable<void> {
    let params = new HttpParams().set('message', req.message);

    return this.http
      .delete<void>(`${this.apiUrl}/users/${id}`, { params })
      .pipe(
        tap(() => {
          const currUsers: UserResponse[] = this.usersSubject.getValue();
