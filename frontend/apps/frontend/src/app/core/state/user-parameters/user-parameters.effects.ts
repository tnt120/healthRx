...
export class UserParametersEffects {
    ...
    return this.actions$.pipe(
      ...
      mergeMap((action) => {
        return this.parameterService
          .setUserParametersMonitor(action.request)
          .pipe(
            map((userParameters) =>
              userParametersActions.setSuccess({ userParameters })
            ),
            catchError((err) => {
              console.error('Error setting user parameters', err);
              this.customSnackbarService.openCustomSnackbar({
                title: 'Niepowodzenie',
                message: 'Wystąpił błąd podczas zapisu parametrów',
                type: 'error',
                duration: 5000,
              });
  ...
  editUserParametersMonitor$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      mergeMap((action) => {
        return this.parameterService
          .editUserParametersMonitor(action.request)
          .pipe(
            map((userParameter) =>
              userParametersActions.editSuccess({ userParameter })
            ),
            catchError((err) => {
              console.error('Error editing user parameters', err);
              this.customSnackbarService.openCustomSnackbar({
                title: 'Niepowodzenie',
                message: 'Wystąpił błąd podczas edycji parametru',
                type: 'error',
                duration: 5000,
              });
