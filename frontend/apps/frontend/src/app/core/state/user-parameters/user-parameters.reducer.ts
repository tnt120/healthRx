...
export const userParametersReducer = createReducer(
  ...
  on(
    ...
    (state, { config }) => config.userParameters || []
  ),
  on(configActions.logout, () => userParametersInitialState),
  on(
    userParametersActions.getSuccess,
    (state, { userParameters }) => userParameters || []
  ),
  on(userParametersActions.setSuccess, (state, { userParameters }) =>
    state.map((up) => {
      const newUserParameter = userParameters.find(
        (upr) => upr.parameter.id === up.parameter.id
      );
  ...
  on(
    userParametersActions.updateSuccess,
    (state, { userParameters }) => userParameters
  )
);
