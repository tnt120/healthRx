...
export class VerificationComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.verificationData$ = this.userService
      .getVerificationData(this.verficationToken)
      .pipe(
        tap((res) => {
          this.data = res;
          if (res.role === Roles.USER) {
            res.parameters.forEach((parameter) => {
              this.userPersonalizationForm.controls.parameters.addControl(
                parameter.id,
        ...
        catchError((err) => {
          if ([301, 305].includes(err.error.code)) {
            const message = this.errorCodesService.getErrorMessage(
              err.error.code
            );
            this.errorMessage.set(message);
          } else {
            this.errorMessage.set(
              'Wystąpił nieznany błąd. Spróbuj ponownie za chwilę'
            );
          }
  ...
  clearHour() {
    this.userPersonalizationForm.controls.parametersNotificationsHour.setValue(
      this.hours[18]
    );
    this.userPersonalizationForm.controls.parametersNotificationsMinute.setValue(
      this.minutes[0]
    );
  }
  ...
  updateErrorMessage(
    ...
    if (control) {
      this.updateFormErrors(
        formName,
        field,
        this.formErrorsService.getErrorMessage(control, name)
      );
    }
  }
  ...
  prepareData() {
    ...
    const parametersForm = this.userPersonalizationForm.get(
      'parameters'
    ) as FormGroup;
    ...
    let data: UserVerificationRequest = {
      ...
      birthDate: this.datePipe.transform(
        personalControls.birthDate.value as unknown as Date,
        'yyyy-MM-dd'
      )!,
    ...
    if (
      this.data.role === Roles.DOCTOR &&
      this.doctorPersonalizationForm.valid
    ) {
      data = {
        ...data,
      ...
      data = {
        ...
        parameters: this.data.parameters.filter((parameter) =>
          Object.keys(parametersForm.value)
            .filter((key) => parametersForm.value[key] === true)
            .includes(parameter.id)
        ),
        parametersNotifications: personalizationControls
          .isParametersNotifications.value
          ? `${personalizationControls.parametersNotificationsHour.value}:${personalizationControls.parametersNotificationsMinute.value}:00`
          : null,
        isBadResultsNotificationsEnabled:
          personalizationControls.isBadResultsNotificationsEnabled.value!,
        isDrugNotificationsEnabled:
          personalizationControls.isDrugNotificationsEnabled.value!,
        ...(personalizationControls.height.value
          ? { height: personalizationControls.height.value }
          : {}),
      };
      ...
      this.userSummary = {
        ...this.userSummary,
        personalData: [
          ...this.userSummary.personalData!,
          {
            title: 'Wzrost',
            value: data.height ? data.height + ' cm' : 'Brak',
          },
        ],
        userNotifications: [
          {
            title: 'Przypomnienie o wpisaniu parametrów',
            value: data.parametersNotifications
              ? 'Tak, o godzinie ' + data.parametersNotifications
              : 'Nie',
          },
          {
            title: 'Powiadomienie o złych wynikach',
            value: data.isBadResultsNotificationsEnabled ? 'Tak' : 'Nie',
          },
          {
            title: 'Powiadamianie o zażyciu leku wysokiego priorytetu',
            value: data.isDrugNotificationsEnabled ? 'Tak' : 'Nie',
          },
        ],
      };
    }
  ...
  verify() {
    ...
    if (this.data.role === Roles.DOCTOR) {
      const profilePicture =
        this.doctorPersonalizationForm.controls.profilePictureUrl.value!;
      const idPhotoFront =
        this.doctorPersonalizationForm.controls.idPhotoFrontUrl.value!;
      const idPhotoBack =
        this.doctorPersonalizationForm.controls.idPhotoBackUrl.value!;
      const types = [
        ImageType.PROFILE,
        ImageType.FRONT_PWZ_PHOTO,
        ImageType.BACK_PWZ_PHOTO,
      ];
      const files = [profilePicture, idPhotoFront, idPhotoBack];
      uploadImagesReq$ = this.imageService.uploadPhotos(
        types,
        files,
        this.data.userEmail
      );
      ...
      if (this.userPersonalizationForm.controls.profilePicture?.value) {
        const profilePicture =
          this.userPersonalizationForm.controls.profilePicture.value!;
        uploadImagesReq$ = this.imageService.uploadPhotos(
          [ImageType.PROFILE],
          [profilePicture],
          this.data.userEmail
    ...
    this.subscription = verifyUserReq$
      .pipe(
      ...
      .subscribe({
        next: () => {
          this.customSnackbarService.openCustomSnackbar({
            title: 'Weryfikacja',
  ...
  changePhoto(event: { file: File; preview: string }, type: string) {
    ...
    switch (type) {
      case ImageType.PROFILE:
        this.doctorPersonalizationForm.controls.profilePictureUrl.setValue(
          event.file
        );
        break;
      case ImageType.FRONT_PWZ_PHOTO:
        this.doctorPersonalizationForm.controls.idPhotoFrontUrl.setValue(
          event.file
        );
        break;
      case ImageType.BACK_PWZ_PHOTO:
        this.doctorPersonalizationForm.controls.idPhotoBackUrl.setValue(
          event.file
        );
        break;
      case 'USER_PROFILE':
        this.userPersonalizationForm.controls.profilePicture.setValue(
          event.file
        );
        break;
    }
