...
import {
  ...
  TakeDrugMonitorDialogData,
} from './../../components/take-drug-monitor-dialog/take-drug-monitor-dialog.component';
import {
...
import {
  ...
  EditUserDrugDialogData,
} from '../../components/edit-user-drug-dialog/edit-user-drug-dialog.component';
...
export class CabinetDashboardComponent implements OnInit, OnDestroy {
  ...
  userDrugsDisplayedColumns: TableColumn[] = [
    ...
    {
      title: 'Forma farmaceutyczna',
      displayedColumn: 'pharmaceuticalFormName',
    },
        ...
        .pipe(
          tap((res) => {
            this.tableData = res.content.map((userDrug) => ({
              ...
              doseDays: userDrug.doseDays
                .map((day) => getDayName(day, true))
                .join(', '),
              doseTimes: userDrug.doseTimes
                .map((time) => time.substring(0, 5))
                .join(', '),
              takingPeriod: `${this.datePipe.transform(
                userDrug.startDate,
                'dd/MM/YYYY'
              )} - ${
                userDrug.endDate
                  ? this.datePipe.transform(userDrug.endDate, 'dd/MM/YYYY')
                  : ''
              }`,
              priority: getPriorityName(userDrug.priority),
              tracking:
                userDrug.amount !== null
                  ? `${userDrug.amount} ${userDrug.drug.unit}`
                  : 'Nie',
            }));
  ...
  onEdit(userDrug: any): void {
    const data: EditUserDrugDialogData = {
      userDrug: this.userDrugs.find((drug) => drug.id === userDrug.id)!,
    };

    const dialogRef: MatDialogRef<EditUserDrugDialogComponent, EditUserDrug> =
      this.dialog.open(EditUserDrugDialogComponent, { data, minWidth: '90vw' });
    ...
    dialogRef.afterClosed().subscribe((res) => {
      if (res) {
        ...
        const request: UserDrugsRequest = {
          ...
          startDate: res.dates?.from
            ? this.datePipe.transform(res.dates.from, 'yyyy-MM-dd')!
            : undefined,
          endDate: res.dates?.to
            ? this.datePipe.transform(res.dates.to, 'yyyy-MM-dd')
            : dateTo,
          amount: res.amount,
          doseTimes: res.times
            ? res.times.map((time) => `${time.hours}:${time.minutes}:00`)
            : undefined,
          doseDays: res.days ? res.days : undefined,
        };

        this.drugsService
          .updateUserDrug(request, data.userDrug.id)
          .subscribe((res) => {
            const data: SnackBarData = {
              title: 'Sukces',
              message: 'Lek został zaktualizowany',
              type: 'success',
              duration: 3000,
            };
            this.customSnackbarService.openCustomSnackbar(data);
            this.userDrugs = this.userDrugs.map((ud) =>
              ud.id === res.id ? res : ud
            );
            this.tableData = this.tableData.map((ud) =>
              ud.id === res.id
                ? {
                    id: res.id,
                    name: res.drug.name,
                    pharmaceuticalFormName: res.drug.pharmaceuticalFormName,
                    doseDays: res.doseDays
                      .map((day) => getDayName(day, true))
                      .join(', '),
                    doseTimes: res.doseTimes
                      .map((time) => time.substring(0, 5))
                      .join(', '),
                    takingPeriod: `${this.datePipe.transform(
                      res.startDate,
                      ...
                      res.endDate
                        ? this.datePipe.transform(res.endDate, 'dd/MM/YYYY')
                        : ''
                    }`,
                    priority: getPriorityName(res.priority),
                    tracking:
                      res.amount !== null
                        ? `${res.amount} ${res.drug.unit}`
                        : 'Nie',
                  }
                : ud
            );
        ...
        .pipe(
          ...
          tap(() => {
            this.drugsService.deleteUserDrug(userDrug.id).subscribe(() => {
              const data: SnackBarData = {
                title: 'Sukces',
                message: 'Lek został usunięty z Twojej apteczki leków',
                type: 'success',
                duration: 3000,
              };
              this.customSnackbarService.openCustomSnackbar(data);
  ...
  onSetDrugMonitor(userDrug: UserDrugMonitorResponse): void {
    ...
    const dialogRef: MatDialogRef<
      TakeDrugMonitorDialogComponent,
      UserDrugMonitorResponse
    > = this.dialog.open(TakeDrugMonitorDialogComponent, {
      data,
      width: '400px',
    });
    ...
    this.subscriptions.add(
      dialogRef
        ...
        .pipe(
          filter((result) => !!result),
          tap((result) => {
            const request: UserDrugMonitorRequest =
              this.prepareMonitorRequest(result);

            this.drugsService.setMonitorDrug(request).subscribe((res) => {
              this.userDrugMonitor.drugsToTake =
                this.userDrugMonitor.drugsToTake.filter(
                  (drug) => !(drug.id === res.id && drug.time === res.time)
                );
              this.userDrugMonitor.drugsTaken = [
                ...this.userDrugMonitor.drugsTaken,
                res,
              ];
  ...
  onEditDrugMonitor(userDrug: UserDrugMonitorResponse): void {
    ...
    const dialogRef: MatDialogRef<
      TakeDrugMonitorDialogComponent,
      UserDrugMonitorResponse | { delete: boolean }
    > = this.dialog.open(TakeDrugMonitorDialogComponent, {
      data,
      width: '400px',
    });
        ...
        .pipe(
          ...
          tap((result) => {
            if ('delete' in result) {
              ...
              this.drugsService
                .deleteMonitorDrug(userDrug.drug.id, userDrug.time)
                .subscribe(() => {
                  this.userDrugMonitor.drugsTaken =
                    this.userDrugMonitor.drugsTaken.filter(
                      (drug) =>
                        !(
                          drug.id === userDrug.id && drug.time === userDrug.time
                        )
                    );
                  this.userDrugMonitor.drugsToTake = [
                    ...this.userDrugMonitor.drugsToTake,
                    { ...userDrug, takenTime: null },
                  ];
              ...
              this.drugsService.editMonitorDrug(request).subscribe((res) => {
                this.userDrugMonitor.drugsTaken =
                  this.userDrugMonitor.drugsTaken.map((userDrug) =>
                    userDrug.id === res.id && userDrug.time === res.time
                      ? res
                      : userDrug
                  );
                this.cdRef.detectChanges();
  ...
  private updateTableAmount(
    ...
    if (findedUserDrug.amount !== null) {
      ...
      this.userDrugs = this.userDrugs.map((ud) =>
        ud.id === userDrug.id ? findedUserDrug : ud
      );
  ...
  prepareMonitorRequest(
    result: UserDrugMonitorResponse
  ): UserDrugMonitorRequest {
    return {
      id: result.id,
