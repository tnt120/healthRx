...
export class MyDoctorsComponent implements OnInit, OnDestroy {
  ...
  isFriendshipsPendingLoading$ =
    this.friendshipService.loadingFriendshipsPending$;

  isFriendshipsRejectedLoading$ =
    this.friendshipService.loadingFriendshipsRejected$;
  ...
  onDelete(friendshipId: string, status: 'WAITING' | 'REJECTED' | 'ACCEPTED') {
    if (status === FriendshipStatus.WAITING) {
      this.friendshipsPending = this.friendshipsPending.filter(
        (f) => f.friendshipId !== friendshipId
      );
    } else if (status === FriendshipStatus.REJECTED) {
      this.friendshipsRejected = this.friendshipsRejected.filter(
        (f) => f.friendshipId !== friendshipId
      );
    } else {
  ...
  onResend(friendshipId: string) {
    const friendship = this.friendshipsRejected.find(
      (f) => f.friendshipId === friendshipId
    );
    if (friendship) {
      this.friendshipsPending.push({
        ...friendship,
