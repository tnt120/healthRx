...
import {
  ...
  DateRangeType,
  DateService,
} from '../../../../core/services/date/date.service';
import { DatePipe } from '@angular/common';
import { MatDialog } from '@angular/material/dialog';
import {
  ManageUserActivityData,
  ManageUserActivityDialogComponent,
} from '../manage-user-activity-dialog/manage-user-activity-dialog.component';
...
export class UserActivitiesComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.isLoading$ = this.activityService.getLoadingActivityState(
      this.isToday() ? 'today' : 'all'
    );
    ...
    if (this.isToday()) {
      ...
      this.date.set({
        label: this.dateRangeOptions[0].value,
        from: this.dateService.getDateRange(this.dateRangeOptions[0].value)
          .from,
    ...
    this.subscriptions.push(
      this.activityService.getFilterChange().subscribe((res) => {
        if (res === this.isToday()) {
          this.loadActivities();
        }
      })
  ...
  loadActivities(): void {
    this.searchParams.startDate =
      this.datePipe.transform(this.date().from, 'yyyy-MM-dd')! + 'T00:00:00';
    this.searchParams.endDate =
      this.datePipe.transform(this.date().to, 'yyyy-MM-dd')! + 'T23:59:59';
    ...
    this.subscriptions.push(
      this.activityService
        ...
        .subscribe((res) => {
          this.tableData.set(
            res.content.map((activity) => this.setTableRow(activity))
          );
  ...
  setTableRow(activity: UserActivityResponse): any {
    return {
      ...
      activityTime: this.datePipe.transform(
  ...
  onEdit(activity: any): void {
    const userActivity: UserActivityResponse = this.isToday()
      ? activity
      : this.userActivities().find((a) => a.id === activity.id);
    ...
    const dialogRef = this.dialog.open(ManageUserActivityDialogComponent, {
      data: dialogData,
      minWidth: '50vw',
    });
    ...
    this.subscriptions.push(
      dialogRef.afterClosed().subscribe((res) => {
        if (res) {
          ...
          this.subscriptions.push(
            this.activityService
              .editUserActivity(userActivity.id, req)
              .subscribe((res) => {
                if (this.isToday()) {
                  if (
                    this.datePipe.transform(res.activityTime, 'YYYY/MM/dd') !==
                    ...
                    this.userActivities.set(
                      this.userActivities().filter((a) => a.id !== res.id)
                    );
                  } else {
                    this.userActivities.set(
                      this.userActivities().map((a) =>
                        a.id === res.id ? res : a
                      )
                    );
                  }
                  ...
                  this.tableData.set(
                    this.tableData().map((a) =>
                      a.id === res.id ? this.setTableRow(res) : a
                    )
                  );
                  this.userActivities.set(
                    this.userActivities().map((a) =>
                  ...
                  if (
                    this.dateService.checkIfToday(res.activityTime) ||
                    this.dateService.checkIfToday(userActivity.activityTime)
                  ) {
      ...
      dialogRef.afterClosed().subscribe((res) => {
        if (res) {
          this.activityService.deleteUserActivity(activity.id).subscribe(() => {
            if (this.isToday()) {
              this.userActivities.set(
                this.userActivities().filter((a) => a.id !== activity.id)
              );
              this.activityService.emitFilterChange();
            } else {
              const findUserActivity = this.userActivities().find(
                (a) => a.id === activity.id
              )!;
              this.tableData.set(
                this.tableData().filter((a) => a.id !== activity.id)
              );
              this.userActivities.set(
                this.userActivities().filter((a) => a.id !== activity.id)
              );
              ...
              if (
                this.dateService.checkIfToday(findUserActivity.activityTime)
              ) {
                this.activityService.emitFilterChange(true);
              }
            }
  ...
  sortOptionMapper(option: string): string {
    switch (option) {
      case 'activityTime':
        return 'Data aktywności';
      case 'duration':
        return 'Czas trwania';
      case 'averageHeartRate':
        return 'Średnie tętno';
      case 'caloriesBurned':
