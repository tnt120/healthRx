...
export class ParameterStatisticsComponent implements OnInit, OnDestroy {
    ...
    this.subscriptions.push(
      this.statisticsService.getParameterStatistics(req).subscribe((res) => {
        ...
        this.tableData = res
          ...
          .map((stat) => ({
            name: `${stat.parameter.name} ${
              stat.parameter.name === 'Waga' ? `(BMI)` : ''
            } [${stat.parameter.unit}]`,
            standards: `${stat.parameter.minStandardValue} - ${stat.parameter.maxStandardValue}`,
            avg: `${Math.round(stat.avgValue * 100) / 100} ${
              stat.parameter.name === 'Waga'
                ? `(${this.parametersService.calculateBmi(
                    this.userHeight(),
                    stat.avgValue
                  )})`
                : ''
            ...
            outsideTheNormDays: `${
              stat.daysBelowMinValue + stat.daysAboveMaxValue
            } (${stat.daysBelowMinValue}/${stat.daysAboveMaxValue})`,
            ...
            iconTrend: {
              icon: this.getTrendIcon(
                TrendType[stat.trend as unknown as keyof typeof TrendType]
              ),
              tooltip:
                TrendType[stat.trend as unknown as keyof typeof TrendType],
  ...
  fillColums(): void {
    this.columns = [
      ...
      {
        title: 'Dni poza normą (poniżej/powyzej)',
        displayedColumn: 'outsideTheNormDays',
      },
      { title: 'Pierwszy pomiar', displayedColumn: 'firstLogDate' },
      { title: 'Ostatni pomiar', displayedColumn: 'lastLogDate' },
      {
        title: 'Dni bez pomiarów (najdłuższa przerwa)',
        displayedColumn: 'daysWithoutLogs',
      },
