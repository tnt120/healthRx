...
export class EditUserDrugDialogComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.data.set({
      ...
      dates: {
        from: new Date(this.dialogData.userDrug.startDate),
        to: this.dialogData.userDrug.endDate
          ? new Date(this.dialogData.userDrug.endDate)
          : null,
      },
      times: this.dialogData.userDrug.doseTimes.map((time) => ({
  ...
  compareValues(key: string, prevValue: any, currentValue: any): boolean {
    if (key === 'dates') {
      prevValue = {
        ...
        to: prevValue.to
          ? prevValue.to.toLocaleString().substring(0, 10)
          : null,
      };

      currentValue = {
        from: currentValue.from
          ? currentValue.from.toLocaleString().substring(0, 10)
          : null,
        to: currentValue.to
          ? currentValue.to.toLocaleString().substring(0, 10)
          : null,
      };

      return !(
        prevValue.from === currentValue.from && prevValue.to === currentValue.to
      );
    }

    if (key === 'times') {
      prevValue = prevValue.map(
    ...
    if (Array.isArray(prevValue) && Array.isArray(currentValue)) {
      ...
      return !prevValue
        .sort()
        .every((val, index) => val === currentValue.sort()[index]);
    }
  ...
  checkValidity(): boolean {
    ...
    const timesNotEqual = times
      .map((time) => `${time.hours}:${time.minutes}`)
      .some((time, i, arr) => arr.indexOf(time) !== i);
  ...
  getModifiedValues(): EditUserDrug {
    ...
    Object.keys(this.prevData).forEach((key) => {
      ...
      if (key === 'amount')
        return (modifiedValues[key as keyof EditUserDrug] = currentValue);
