...
import {
  ...
  EditDrugStockDialogData,
} from '../edit-drug-stock-dialog/edit-drug-stock-dialog.component';
  ...
  animations: [
    trigger('animateExpand', [
      transition(':enter', [
        ...
        animate(
          '.3s ease',
          style({ opacity: 1, height: '*', 'padding-top': '*' })
        ),
      ]),
      transition(':leave', [
        animate(
...
export class UserDrugsDetailsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.subscriptions.push(
      this.drugService.getDrugPacks(this.selectedDrug().id).subscribe((res) => {
  ...
  generateHoursAndMinutes() {
    ...
    this.minutes = Array.from({ length: 60 }, (v, k) =>
      k < 10 ? '0' + k : k.toString()
    );
  }
  ...
  editStock(): void {
    ...
    const dialogRef: MatDialogRef<EditDrugStockDialogComponent, number> =
      this.dialog.open(EditDrugStockDialogComponent, { data, width: '800px' });
  ...
  toggleDay(isChecked: boolean, day: string): void {
    if (!isChecked) {
      this.data.set({
        ...this.data(),
        days: [...this.data().days].filter((d) => d !== day),
      });
    } else {
