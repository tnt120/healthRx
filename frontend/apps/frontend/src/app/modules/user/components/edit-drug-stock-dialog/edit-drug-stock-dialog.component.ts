...
export class EditDrugStockDialogComponent implements OnInit {
  ...
  getAmount(): number {
    if (this.isPackAdd() && this.isValid()) {
      return (
        ...
        this.selectedPack()!.packSize * this.numberOfPacks() -
        (this.selectedPack()!.packSize - this.firstPackAmount())
      );
    }
  ...
  isValid(): boolean {
    if (!this.isPackAdd())
      return this.dialogData.amount !== null && this.dialogData.amount >= 0;
