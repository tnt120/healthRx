...
export class ActivityStatisticsComponent {
  ...
  getActivityStats(): void {
    ...
    this.subscriptions.push(
      this.statisticsService.getAcitvityStatistics(req).subscribe((res) => {
        ...
        this.tableData = res.map((stat) => ({
          name: stat.activity.name,
          firstLogDate: stat?.firstLogDate?.substring(0, 10),
          lastLogDate: stat?.lastLogDate?.substring(0, 10),
          minDuration: stat.avgDuration
            ? this.activityService.getActivityDuration(stat.minDuration)
            : '-',
          maxDuration: stat.avgDuration
            ? this.activityService.getActivityDuration(stat.maxDuration)
            : '-',
          avgDuration: stat.avgDuration
            ? this.activityService.getActivityDuration(stat.avgDuration)
            : '-',
          heartRate: stat.avgHeartRate
            ? stat.maxHeartRate +
              ' / ' +
              stat.minHeartRate +
              ' / ' +
              stat.avgHeartRate
            : '- / - / -',
          caloriesBurned: stat.avgCaloriesBurned
            ? stat.maxCaloriesBurned +
              ' / ' +
              stat.minCaloriesBurned +
              ' / ' +
              stat.avgCaloriesBurned
            : '- / - / -',
  ...
  fillColumns(): void {
    this.columns = [
      ...
      {
        title: 'Spalone kalorie (max/min/śr)',
        displayedColumn: 'caloriesBurned',
      },
    ];
  }
