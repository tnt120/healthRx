...
export class DrugStatisticsComponent implements OnInit, OnDestroy {
      ...
      this.statisticsService.getDrugsStatistics(req).subscribe((res) => {
        ...
        this.tableData = res
          ...
          .map((stat) => ({
            ...
            punctuallyPercentage:
              Math.round(stat.punctualityPercentage * 100) / 100,
  ...
  fillColumns(): void {
    this.columns = [
      ...
      {
        title: 'Procent zażycia leku [%]',
        displayedColumn: 'compliancePercentage',
      },
      { title: 'Pierwsze zażycie', displayedColumn: 'firstLogDate' },
      { title: 'Ostatnie zażycie', displayedColumn: 'lastLogDate' },
      {
        title: 'Dni z zażytymi wszystkimi dawkami (częścią dawek)',
        displayedColumn: 'daysTaken',
      },
      ...
      {
        title: 'Procent punktualności [%]',
        displayedColumn: 'punctuallyPercentage',
      },
