...
export class ParameterChartComponent implements OnInit, OnDestroy {
  ...
  isChartLoading$ = this.statisticsService.getLoadingChartState(
    StatisticsType.PARAMETER
  );

  chartData = signal<(ChartResponse & { selectedParameter: Parameter }) | null>(
    null
  );
  ...
  parameters$: Observable<Parameter[]> = this.store
    .select('userParameters')
    .pipe(
      map((userParam) =>
        userParam.map((param: UserParameterResponse) => param.parameter)
      )
    );
