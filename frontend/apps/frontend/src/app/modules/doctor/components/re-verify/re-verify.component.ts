...
export class ReVerifyComponent implements OnInit {
  ...
  previewImage = signal<{
    [key in Exclude<ImageType, ImageType.PROFILE>]: File | null;
  }>({
  ...
  ngOnInit(): void {
    this.reVerifyForm.controls.numberPESEL.setValue(
      this.user().unverifiedDoctor!.numberPESEL!
    );
    this.reVerifyForm.controls.numberPWZ.setValue(
      this.user().unverifiedDoctor!.numberPWZ!
    );
  }
  ...
  onSubmit(): void {
    ...
    const uploadImagesReq$ = this.imageService.uploadPhotos(
      types,
      files,
      this.user().email
    );
      ...
      .pipe(
        ...
        tap(() => {
          this.customSnackbarService.openCustomSnackbar({
            title: 'Sukces',
            message: 'Pomyślnie wysłano dane',
            type: 'success',
            duration: 2500,
          });
