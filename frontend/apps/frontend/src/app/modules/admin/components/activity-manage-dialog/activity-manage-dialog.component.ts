...
export class ActivityManageDialogComponent implements OnInit {
  ...
  manageActivityForm = this.fb.group({
    ...
    metFactor: this.fb.control<number | null>(null, [
      Validators.required,
      Validators.min(0),
    ]),
  ...
  ngOnInit(): void {
    if (this.data.activity) {
      ...
      this.manageActivityForm
        .get('metFactor')
        ?.setValue(this.data.activity.metFactor);
      this.manageActivityForm
        .get('isPopular')
        ?.setValue(this.data.activity.isPopular);
  ...
  protected isValid(): boolean {
    ...
    return (
      ...
      JSON.stringify(this.manageActivityForm.value)
    );
  }
}
