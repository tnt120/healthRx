...
export class ParameterMangeDialogComponent implements OnInit {
  ...
  manageParameterForm = this.fb.group({
    ...
    minStandardValue: this.fb.control<number | null>(null, [
      Validators.required,
      Validators.min(0),
    ]),
    maxStandardValue: this.fb.control<number | null>(null, [
      Validators.required,
      Validators.min(0),
    ]),
  ...
  ngOnInit(): void {
    if (this.data.parameter) {
      ...
      const unitId: string = this.data.units.find(
        (unit) => unit.symbol === this.data.parameter!.unit
      )!.id;
      this.manageParameterForm.get('unitId')?.setValue(unitId);
      this.manageParameterForm
        .get('minStandardValue')
        ?.setValue(this.data.parameter.minStandardValue);
      this.manageParameterForm
        .get('maxStandardValue')
        ?.setValue(this.data.parameter.maxStandardValue);
      this.manageParameterForm
        .get('minValue')
        ?.setValue(this.data.parameter.minValue);
      this.manageParameterForm
        .get('maxValue')
        ?.setValue(this.data.parameter.maxValue);
  ...
  protected isValid(): boolean {
    ...
    const valuesValid =
      this.manageParameterForm.get('minStandardValue')?.value! <=
        this.manageParameterForm.get('maxStandardValue')?.value! &&
      this.manageParameterForm.get('minValue')?.value! <=
        this.manageParameterForm.get('maxValue')?.value! &&
      this.manageParameterForm.get('minValue')?.value! <=
        this.manageParameterForm.get('minStandardValue')?.value! &&
      this.manageParameterForm.get('maxValue')?.value! >=
        this.manageParameterForm.get('maxStandardValue')?.value!;
    ...
    return (
      valuesValid &&
      JSON.stringify(this.manageParameterForm.value) !==
        JSON.stringify(this.onStartValue())
    );
  }
}
