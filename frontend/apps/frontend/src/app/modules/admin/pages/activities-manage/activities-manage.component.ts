...
import {
  ...
  ActivityManageDialogData,
} from '../../components/activity-manage-dialog/activity-manage-dialog.component';
...
export class ActivitiesManageComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.activities$ = this.adminService
      .getActivities()
      .pipe(tap((res) => this.activities.set(res)));
  ...
  onAdd(): void {
    ...
    const dialogRef = this.dialog.open(ActivityManageDialogComponent, {
      data,
      minWidth: '350px',
    });
        ...
        .pipe(
          ...
          map((res) => {
            const req: Partial<ActivityRequest> = {
              ...
              metFactor:
                res.metFactor === activity.metFactor
                  ? undefined
                  : res.metFactor,
              isPopular:
                res.isPopular === activity.isPopular
                  ? undefined
                  : res.isPopular,
            };
  ...
  private handleActivityEdit(res: Partial<ActivityRequest>, id: string): void {
    this.adminService
      ...
      .pipe(
        tap((activity) => {
          this.activities.set(
            this.activities()?.map((a) => (a.id === id ? activity : a)) || []
          );
        })
      )
      .subscribe();
