...
import {
  ...
  ParameterMangeDialogData,
} from '../../components/parameter-mange-dialog/parameter-mange-dialog.component';
...
export class ParametersManageComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.parametersAndUnits$ = this.adminService
      .getParameters()
      .pipe(tap((res) => this.parametersAndUnits.set(res)));
  ...
  onAdd(): void {
    ...
    const dialogRef = this.dialog.open(ParameterMangeDialogComponent, {
      data,
      minWidth: '350px',
    });
        ...
        .pipe(
          ...
          map((res) => {
            const req: Partial<ParameterRequest> = {
              ...
              unitId:
                res.unitId ===
                this.parametersAndUnits()!.units.find(
                  (unit) => unit.symbol === param.unit
                )!.id
                  ? undefined
                  : res.unitId,
              minStandardValue:
                res.minStandardValue === param.minStandardValue
                  ? undefined
                  : res.minStandardValue,
              maxStandardValue:
                res.maxStandardValue === param.maxStandardValue
                  ? undefined
                  : res.maxStandardValue,
              minValue:
                res.minValue === param.minValue ? undefined : res.minValue,
              maxValue:
                res.maxValue === param.maxValue ? undefined : res.maxValue,
            };
      ...
      .pipe(
        tap((param) => {
          this.parametersAndUnits.set({
            parameters: [
              ...(this.parametersAndUnits()?.parameters || []),
              param,
            ],
            units: this.parametersAndUnits()?.units || [],
          });
  ...
  private handleParameterEdit(
    res: Partial<ParameterRequest>,
    id: string
  ): void {
    this.adminService
      .editParameter(res, id)
      .pipe(
        tap((param) => {
          this.parametersAndUnits.set({
            parameters:
              this.parametersAndUnits()?.parameters.map((p) =>
                p.id === param.id ? param : p
              ) || [],
            units: this.parametersAndUnits()?.units || [],
          });
        })
    ...
    this.adminService
      ...
      .pipe(
        tap(() => {
          this.parametersAndUnits.set({
            parameters:
              this.parametersAndUnits()?.parameters.filter(
                (p) => p.id !== param.id
              ) || [],
            units: this.parametersAndUnits()?.units || [],
          });
