...
export class UserProfileEditDialogComponent implements OnInit {
  ...
  isValid(): boolean {
    const basicTest =
      this.data.value !== this.currValue &&
      this.currValue !== '' &&
      this.currValue !== null;
    ...
    if (this.data.profilParameter === 'birthDate') {
      return (
        this.data.value !== this.formatDate(this.currValue.toLocaleString())
      );
    }
    ...
    if (
      this.data.profilParameter === 'height' &&
      typeof this.currValue === 'number'
    ) {
      return (
        this.data.value !== this.currValue &&
        this.currValue >= 100 &&
