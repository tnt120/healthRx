...
export class NotificationsManageDialogComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.currValues.set({
      ...
      isBadResultsNotificationsEnabled:
        this.data.isBadResultsNotificationsEnabled,
      isDrugNotificationsEnabled: this.data.isDrugNotificationsEnabled,
      hours: this.data.parametersNotifications
        ? this.data.parametersNotifications.split(':')[0]
        : '18',
      minutes: this.data.parametersNotifications
        ? this.data.parametersNotifications.split(':')[1]
        : '00',
    });
  ...
  isValid(): boolean {
    const parametersNotificationsCheck = this.data.parametersNotifications
      ? this.data.parametersNotifications !==
          `${this.currValues().hours}:${this.currValues().minutes}:00` ||
        !this.currValues().parametersNotifications
      : !!this.currValues().parametersNotifications;

    return (
      this.currValues().isBadResultsNotificationsEnabled !==
        this.data.isBadResultsNotificationsEnabled ||
      this.currValues().isDrugNotificationsEnabled !==
        this.data.isDrugNotificationsEnabled ||
  ...
  getValue(): NotificationsData {
    return {
      parametersNotifications: this.currValues().parametersNotifications
        ? `${this.currValues().hours}:${this.currValues().minutes}:00`
        : null,
      isBadResultsNotificationsEnabled:
        this.currValues().isBadResultsNotificationsEnabled,
