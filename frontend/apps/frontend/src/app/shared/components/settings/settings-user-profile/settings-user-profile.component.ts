...
} from './../user-profile-edit-dialog/user-profile-edit-dialog.component';
...
export class SettingsUserProfileComponent implements OnDestroy {
  ...
  getHeight(): string {
    return this.userProfileData()?.height
      ? this.userProfileData()?.height + ' cm'
      : 'Brak';
  }

  onEdit(
    field: keyof Omit<UserResponse, 'isDoctorVerified' | 'unverifiedDoctor'>
  ): void {
    ...
    const dialogRef = this.dialog.open(UserProfileEditDialogComponent, {
      data: dialogData,
      width: '400px',
    });
          ...
          this.subscriptions.push(
            this.settingsSercice.persinalDataChange(request).subscribe(() => {
              this.userProfileData.set({
                ...(this.userProfileData() as UserResponse),
                [field]: result,
              });

              this.store.dispatch(
                userActions.editSuccess({
                  userData: this.userProfileData() as UserResponse,
                })
              );
            })
          );
