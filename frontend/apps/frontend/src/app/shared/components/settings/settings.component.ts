...
export class SettingsComponent implements OnInit, OnDestroy {
  ...
  notificationsSettings$: Observable<NotificationsData> = this.store.select(
    'notificationsSettings'
  );

  userParamters$: Observable<UserParameterResponse[]> =
    this.store.select('userParameters');

  paramters$: Observable<Parameter[]> = this.store.select('parameters');

  ngOnInit(): void {
    this.parametersChange =
      this.route.snapshot?.queryParams['parametersChange'];
    ...
    this.observer.observe('(max-width: 1024px)').subscribe((res) => {
      res.matches
        ? (this.tabPadding = '24px 10px')
        : (this.tabPadding = '10px 24px');
    });
  }
