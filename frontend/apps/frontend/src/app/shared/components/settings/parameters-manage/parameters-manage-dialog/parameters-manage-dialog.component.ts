...
export class ParametersManageDialogComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.selectedParameters = this.data.parameters.filter((p) =>
      this.data.userParameters.some((up) => up.id === p.id)
    );
    this.currentSelectedParameters = [...this.selectedParameters];

    this.data.parameters.forEach((p) => {
      this.currParametersForm.addControl(
        p.id,
        new FormControl(this.selectedParameters.includes(p))
      );
    });

    this.currParametersForm.valueChanges.subscribe(
  ...
  isValid(): boolean {
    return (
      this.selectedParameters.length !==
        this.currentSelectedParameters.length ||
      this.selectedParameters.some(
        (p) => !this.currentSelectedParameters.some((cp) => cp.id === p.id)
      )
    );
  }
}
