...
import {
  ...
  ParametersManageDialogData,
} from './parameters-manage-dialog/parameters-manage-dialog.component';
...
export class ParametersManageComponent implements OnInit, OnDestroy {
  ...
  onEdit() {
    const data: ParametersManageDialogData = {
      userParameters: this.userParameters()?.map((up) => up.parameter)!,
      parameters: this.parameters()!,
    };

    const widthString = this.isMobile() ? '100%' : '738px';

    const dialogRef = this.dialog.open(ParametersManageDialogComponent, {
      data,
      ...
      dialogRef.afterClosed().subscribe((res) => {
        if (res) {
          this.parametersService.editUserParameters(res).subscribe((resp) => {
            ...
            this.store.dispatch(
              userParametersActions.updateSuccess({ userParameters: resp })
            );
          });
        }
      })
