    ...
    this.subscriptions.push(
      this.imageService.getPictures(req).subscribe((res) => {
        if (res?.[0]?.imageType === ImageType.PROFILE && res[0]?.image) {
          this.currPicturePreview.set(
            'data:image/jpeg;base64 ,' + res[0].image
          );
          this.profilePicturePreview.set(
            'data:image/jpeg;base64 ,' + res[0].image
          );
        }
      })
    ...
    this.subscriptions.push(
      this.imageService.deleteProfilePicture().subscribe({
        next: () => {
          ...
          this.customSnackbarService.openCustomSnackbar({
            title: 'Sukces',
            message: 'Pomyślnie usunięto zdjęcie profilowe.',
            type: 'success',
            duration: 2500,
          });
        },
