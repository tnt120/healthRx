...
export class BarChartComponent implements OnInit {
  ...
  generateChart(): void {
    ...
    if (this.labels().length > 1) {
      const mainDataSet = this.chartData()?.data.map((d) =>
        d.value > 0 ? d.value : 0
      );
      const additionalDataSet = this.chartData()?.data?.map((d) =>
        d?.additionalValue ? d.additionalValue : 0
      );

      labels = this.chartData()?.data.map((d) =>
        getDayName(d.label as Days) ? getDayName(d.label as Days) : 'Całość'
      );
      ...
      const backgrounds = Array(this.chartData()?.data.length)
        .fill('#1469C0')
        .map((color, index) => {
