...
export class ChatComponent implements OnInit, OnDestroy {
  ...
  chatUser$: Observable<UserResponse> = this.store
    .select('user')
    .pipe(tap((user) => (this.chatUser = user)));
  ...
  ngOnInit(): void {
    ...
    this.subscriptions.push(
      this.websocketService.messageReceived$
        .pipe(
          ...
          filter(
            (message) =>
              this.selectedConversation()?.friendshipId ===
                message.friendshipId &&
              !message.isRead &&
              message.receiverId === this.chatUser?.id
          ),
    ...
    this.subscriptions.push(
      this.websocketService.messageRead$
        .pipe(tap((message) => this.updateReadStatus(message)))
        .subscribe()
    );
  }
  ...
  private updateReadStatus(message: ChatMessageDto): void {
    const findedConversation = this.conversations().find(
      (c) => c.friendshipId === message.friendshipId
    );

    if (findedConversation) {
      const findedMessage = findedConversation.messages?.find(
        (m) => m.id === message.id
      );
