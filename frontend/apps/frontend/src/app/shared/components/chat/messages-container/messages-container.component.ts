...
export class MessagesContainerComponent implements OnChanges {
    ...
    if (changes['conversation']) {
      if (this.conversation()?.user?.pictureUrl) {
        this.profilePicture.set(
          'data:image/jpeg;base64 ,' + this.conversation()?.user.pictureUrl
        );
      } else {
  ...
  compareDates(
    currMessage: ChatMessageDto,
    prevMessage: ChatMessageDto | undefined
  ): boolean {
    const currTime = currMessage.createdAt
      ? new Date(currMessage.createdAt).getTime()
      : 0;
