...
import {
  ...
  FriendshipResponse,
} from '../../../core/models/friendship-response.model';
...
export class FriendshipCardComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    if (this.friendship().user?.pictureUrl) {
      this.profilePicture.set(
        'data:image/jpeg;base64 ,' + this.friendship().user.pictureUrl
      );
    }

    this.reportPermisison.set(
      this.friendship().permissions.activitiesAccess ||
        this.friendship().permissions.parametersAccess ||
        this.friendship().permissions.userMedicineAccess
    );
  }
  ...
  approve() {
    this.subscriptions.add(
      this.friendshipService
        .acceptInvitation({ invitationId: this.friendship().friendshipId })
        .subscribe((res) => {
  ...
  reject() {
    this.subscriptions.add(
      this.friendshipService
        .rejectInvitation({ invitationId: this.friendship().friendshipId })
        .subscribe((res) => {
          this.emitAcceptedAndDeleted.emit(res.friendshipId);
        })
    );
  ...
  resend() {
    this.subscriptions.add(
      this.friendshipService
        .resendInvitation({ invitationId: this.friendship().friendshipId })
        .subscribe((res) => {
  ...
  onGenerateClick() {
    ...
    const dialogRef = this.dialog.open(GenerateRaportDialogComponent, {
      data: dialogData,
    });
      ...
      this.statisticsService
        ...
        .pipe(
          tap((res) => {
            ...
            a.href = url;
            a.download = `raport-${this.friendship().user.firstName}-${
              this.friendship().user.lastName
  ...
  cancel() {
    const dialogData: ConfirmationDialogData = {
      ...
      message: `Czy na pewno chcesz usunąć ${
        this.friendship().user.firstName
      } ${this.friendship().user.lastName} ze znajomych?`,
      acceptButton: 'Usuń',
    };
    ...
    this.subscriptions.add(
      dialogRef.afterClosed().subscribe((res) => {
        if (res) {
          ...
          this.subscriptions.add(
            this.friendshipService
              .cancelInvitation(this.friendship().friendshipId, isAccepted)
              .subscribe((res) => {
                this.emitAcceptedAndDeleted.emit(res.friendshipId);
              })
          );
  ...
  editPermissions() {
    const dialogRef = this.dialog.open(
      ...
      { data: this.friendship().permissions }
    );
    ...
    this.subscriptions.add(
      dialogRef.afterClosed().subscribe((res) => {
        if (res) {
          this.subscriptions.add(
            this.friendshipService
              .updatePermissions(this.friendship().friendshipId, res)
              .subscribe()
          );
        }
      })
