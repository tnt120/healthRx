...
export class LineChartComponent implements OnInit {
  ...
  chartData = input.required<
    ChartResponse & { selectedParameter: Parameter }
  >();
  ...
  ngOnInit(): void {
    ...
    combineLatest([
      phoneObserver,
      smallScreenObserver,
      mediumScreenObserver,
    ]).subscribe(([phoneScreen, smallScreen, mediumScreen]) => {
  ...
  generateChart(): void {
    ...
    const minStandardValue =
      this.chartData().selectedParameter.name === 'Waga'
        ? this.parametersService.getMaxWeightFromBmiAndHeight(
            this.userHeight(),
    ...
    const minTemp =
      Math.min(...dataPoints, minStandardValue) -
      (this.chartData().selectedParameter.minStandardValue > 1000 ? 100 : 10);
    const minY = minTemp >= 0 ? minTemp : 0;

    const maxStandardValue =
      this.chartData().selectedParameter.name === 'Waga'
        ? this.parametersService.getMaxWeightFromBmiAndHeight(
            this.userHeight(),
    ...
    const maxTemp =
      Math.max(...dataPoints, maxStandardValue) +
      (this.chartData().selectedParameter.maxStandardValue > 1000 ? 100 : 10);
    const maxY: number =
      maxTemp <= this.chartData().selectedParameter.maxValue
        ? maxTemp
        : this.chartData().selectedParameter.maxValue;
